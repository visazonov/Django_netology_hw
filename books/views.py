...
def books_details(request, pub_date):
    ...
    all_dates = (
        Book.objects.values_list(
            "pub_date", flat=True
        )  # c flat=True список дат, а не список кортежей
        .distinct()  # Убирает повторы дат.
        .order_by("pub_date")  # Сортирует даты по возрастанию
    )

    # предыдущая дата
    prev_pub_date = all_dates.filter(
        pub_date__lt=date
    ).last()  # выбирает даты меньше date  # Берёт последний элемент

    next_pub_date = all_dates.filter(
        pub_date__gt=date
    ).first()  # выбирает даты больше текущей  # берёт первый элемент — ближайшая следующая дата
